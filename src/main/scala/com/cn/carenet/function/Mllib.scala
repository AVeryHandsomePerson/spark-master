...
//      }
  ...
  def LogicalForest(): Unit ={
    val spark = SparkSession.builder().master("local[*]").appName("SparkSqlTest").getOrCreate()
    val data = MLUtils.loadLibSVMFile(spark.sparkContext, "D/IDEARFILE/aaaaa/新建文件夹/sample_libsvm_data.txt")
    ...
    //加载随机森林模型到内存
    val sameModel = RandomForestModel.load(spark.sparkContext, "myModelPath")
  }
